...
export default function JeuxPage() {
  ...
  async function handleSave() {
    setSaving(true)
    if (jeuEdit) {
      await supabase.from('jeux').update({ nom, slug, logo_url: logoUrl, actif }).eq('id', jeuEdit.id)
    } else {
    ...
    <div className="min-h-screen bg-[#0f0f1a]">
      ...
      <main className="ml-64 p-8">
        <div className="flex items-center justify-between mb-8">
          ...
          <Button onClick={openCreate} className="bg-purple-600 hover:bg-purple-700 text-white rounded-xl">
            ...
            {jeux.map((jeu) => (
              <div key={jeu.id} className="bg-white/5 border border-white/5 rounded-2xl p-5">
                ...
                <div className="flex gap-2">
                  <button onClick={() => openEdit(jeu)} className="flex-1 flex items-center justify-center gap-1.5 bg-white/5 hover:bg-white/10 text-slate-300 text-sm py-2 rounded-xl transition-all">
                    <Pencil size={14} /> Modifier
                  </button>
                  <button onClick={() => handleDelete(jeu.id)} className="flex items-center justify-center bg-red-500/10 hover:bg-red-500/20 text-red-400 px-3 rounded-xl transition-all">
                    <Trash2 size={14} />
                  </button>
                </div>
              </div>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-[#13131f] border border-white/10 rounded-2xl w-full max-w-md">
            <div className="flex items-center justify-between px-6 py-5 border-b border-white/5">
              <h3 className="text-white font-semibold">{jeuEdit ? 'Modifier le jeu' : 'Nouveau jeu'}</h3>
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <Input value={nom} onChange={(e) => setNom(e.target.value)} placeholder="Free Fire"
              ...
              <div>
                ...
                <Input value={slug} onChange={(e) => setSlug(e.target.value)} placeholder="free-fire"
                  className="bg-white/10 border-white/20 text-white rounded-xl h-11" />
              </div>
              <div>
                <Label className="text-slate-300 text-sm mb-1.5 block">URL du logo</Label>
                <Input value={logoUrl} onChange={(e) => setLogoUrl(e.target.value)} placeholder="/logos/free-fire.png"
                  className="bg-white/10 border-white/20 text-white rounded-xl h-11" />
              </div>
              <div className="flex items-center gap-2">
                <input type="checkbox" id="actif" checked={actif} onChange={(e) => setActif(e.target.checked)}
                  className="w-4 h-4 rounded accent-purple-600" />
                <Label htmlFor="actif" className="text-slate-300 text-sm">Jeu actif</Label>
              </div>
